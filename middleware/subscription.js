...
const { getUserSubscription, checkDailyUsage } = require('../models/subscription');
...
function requireFeature(requiredFeature) {
    return async (req, res, next) => {
        try {
            ...
            console.log(`[subscriptionMiddleware] Checking feature access: ${requiredFeature} for user: ${user_id}`);
            ...
            console.error('[subscriptionMiddleware] Error checking feature access:', error);
            res.status(500).json({
...
function checkDailyLimit() {
    return async (req, res, next) => {
        try {
            ...
            console.log(`[subscriptionMiddleware] Checking daily usage limit for user: ${user_id}`);
            ...
            console.error('[subscriptionMiddleware] Error checking daily limit:', error);
            res.status(500).json({
...
function requireTierLevel(minTierLevel) {
    return async (req, res, next) => {
        try {
            ...
            console.log(`[subscriptionMiddleware] Checking tier level: ${minTierLevel} for user: ${user_id}`);
            ...
            console.error('[subscriptionMiddleware] Error checking tier level:', error);
            res.status(500).json({
...
function checkFileUploadLimit(fileSize) {
    return async (req, res, next) => {
        try {
            ...
            console.log(`[subscriptionMiddleware] Checking file upload limit for user: ${user_id}, file size: ${fileSize} bytes`);
            ...
            const maxFileSize = subscription.data.tier.max_file_upload_size * 1024 * 1024; // MB to bytes
            ...
            console.error('[subscriptionMiddleware] Error checking file upload limit:', error);
            res.status(500).json({
