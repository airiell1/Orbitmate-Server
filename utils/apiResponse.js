 ...
 */
function createSearchApiResponse(success, data = null, message = '', errorCode = null) {
  if (success && data) {
    // 성공시: 데이터만 반환 (snake_case 변환)
    return Array.isArray(data) ? data.map(item => toSnakeCaseObj(item)) : toSnakeCaseObj(data);
  } else {
    // 실패시: 에러 메시지 반환
