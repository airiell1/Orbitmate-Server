...
async function getSubscriptionTiers() {
    ...
    try {
          ...
          const tiers = result.rows.map(row => {
            ...
            if (row[9]) {
                try {
                    ...
                    console.warn('[subscriptionModel] Failed to parse features_included:', row[9], parseError);
                    featuresIncluded = [];
                }
            }
        ...
        console.error('[subscriptionModel] Error getting subscription tiers:', error);
        throw error;
    } finally {
        if (connection) {
    ...
    try {
        ...
        if (row[18]) {
            try {
                ...
                console.warn('[subscriptionModel] Failed to parse features_included:', row[18], parseError);
                featuresIncluded = [];
            }
        }
        ...
        console.error('[subscriptionModel] Error getting user subscription:', error);
        throw error;
    } finally {
        if (connection) {
            await connection.close();
...
async function createDefaultSubscription(user_id) {
    ...
    try {
        ...
        console.error('[subscriptionModel] Error creating default subscription:', error);
        throw error;
    } finally {
        if (connection) {
            await connection.close();
...
async function updateUserSubscription(user_id, new_tier_name, options = {}) {
    ...
    try {
        ...
        console.log(`[subscriptionModel] Mapped tier name: ${new_tier_name} -> ${englishTierName}`);
        ...
        `;
        const tierResult = await connection.execute(tierQuery, { tier_name: englishTierName });
          if (tierResult.rows.length === 0) {
            throw new Error(`Subscription tier '${new_tier_name}' (mapped to '${englishTierName}') not found`);
        }
        
        const [new_tier_id, tier_level, monthly_price, yearly_price] = tierResult.rows[0];
        ...
        console.log(`[subscriptionModel] User ${user_id} subscription updated to ${new_tier_name}`);
        ...
        console.error('[subscriptionModel] Error updating user subscription:', error);
        throw error;
    } finally {
        if (connection) {
            await connection.close();
...
async function getUserSubscriptionHistory(user_id) {
    ...
    try {
        ...
        console.error('[subscriptionModel] Error getting subscription history:', error);
        throw error;
    } finally {
        if (connection) {
            await connection.close();
...
async function checkDailyUsage(user_id) {
    ...
    try {
        ...
        return standardizeApiResponse({
            ...
            requests_remaining: maxRequests ? Math.max(0, maxRequests - todayRequests) : null,
