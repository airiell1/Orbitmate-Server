...
router.get('/', (req, res) => {
  res.send(`
    ...
    <body>
      ...
      <script>
      // API 목록 정의 (설명, 예시, 파라미터 등)
      // 아래는 api.txt 기반 자동 생성 (일부 예시, 전체는 필요시 확장)
      const apis = [
        {
          method: 'GET',
          path: '/api/health',
        ...
        },
        {
          ...
          title: '로그인',
          desc: '사용자가 로그인합니다.<br><span class="api-desc-note">email에 "API@example.com", password에 "password123"을 입력하면 userId는 항상 "API_TEST_USER_ID"로 고정됩니다.</span>',
          params: [
            { name: 'email', type: 'email', label: '이메일', required: true },
            { name: 'password', type: 'password', label: '비밀번호', required: true }
          ],
        ...
        /* 3. 채팅 세션 관리 */
        {
          ...
          title: '채팅 세션 생성',
          desc: '새로운 채팅 세션을 생성합니다.<br><span class="api-desc-note">userId에 "API_TEST_USER_ID"를 입력하면 테스트 계정으로 세션을 생성할 수 있습니다.</span>',
          params: [
            { name: 'userId', type: 'text', label: '사용자 ID', required: true },
        ...
        /* 4. 채팅 메시지 관리 */
        {
          ...
          params: [
            ...
            { name: 'systemPrompt', type: 'text', label: '시스템 프롬프트', required: false },
            { name: 'specialModeType', type: 'text', label: '특수 모드', required: false },
            { name: 'stream', type: 'checkbox', label: '스트리밍 응답', required: false }
          ],
          exampleReq:  \`{\n  "message": "안녕! 오늘 날씨 어때?",\n  "systemPrompt": "(선택) AI 행동 지시",\n  "specialModeType": "(선택) stream/canvas 등",\n  "stream": true\n}\`,
        ...
        /* 5. 파일 업로드 */
        {
          ...
          params: [
            ...
            { name: 'userId', type: 'text', label: '사용자 ID', required: false }
          ],
          exampleReq: '(multipart/form-data)',
      ...
      apis.forEach((api, idx) => {
        ...
        // 디폴트값 자동 입력: 테스트 계정/세션/메시지 관련 파라미터는 고정값
        const defaultValues = {
          'username': 'APItest',
          'email': 'API@example.com',
          'password': 'password123',
