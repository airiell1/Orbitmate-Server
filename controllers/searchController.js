...
async function searchWeatherController(req, res) {
    try {
        ...
        console.log(`[searchController] Weather search requested from IP: ${clientIP}`);
        ...
        console.log(`[searchController] Getting weather: units=${units}, lang=${lang}, city=${city || 'auto'}`);

        // 날씨 정보 조회
        const weatherData = await getWeatherByIP(clientIP, { units, lang, city, lat, lon });
        
        console.log(`[searchController] Weather data retrieved successfully for location: ${weatherData.location?.name || 'Unknown'}`);
        ...
        if (error.code === 'LOCATION_NOT_FOUND') {
            return res.status(404).json(createSearchApiResponse(
                false,
                null,
