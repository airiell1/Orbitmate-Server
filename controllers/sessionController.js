...
async function createSessionController(req, res) { // todo: 인자 3개로 수정하기
  ...
  try {
    ...
    if (!session || !session.session_id) {
        ...
        const errorPayload = createErrorResponse('SERVER_ERROR', '세션 생성에 실패했습니다.'); // Or a more specific code if available
        return res.status(getHttpStatusByErrorCode('SERVER_ERROR')).json(standardizeApiResponse(errorPayload));
    }
    ...
    const errorPayload = createErrorResponse('SERVER_ERROR', `세션 생성 중 오류 발생: ${err.message}`);
...
async function getUserSessionsController(req, res) {
  ...
  */
  ...
  try {
    ...
    const errorPayload = createErrorResponse('SERVER_ERROR', `세션 목록 조회 중 오류 발생: ${err.message}`);
    res.status(getHttpStatusByErrorCode('SERVER_ERROR')).json(standardizeApiResponse(errorPayload));
  }
}
...
async function updateSessionController(req, res) {
  ...
  try {
    ...
    if (!updatedSession) {
      ...
      const errorPayload = createErrorResponse('SESSION_NOT_FOUND', '세션을 찾을 수 없거나 업데이트되지 않았습니다.');
    ...
    const errorPayload = createErrorResponse('SERVER_ERROR', `세션 수정 중 오류 발생: ${err.message}`);
    res.status(getHttpStatusByErrorCode('SERVER_ERROR')).json(standardizeApiResponse(errorPayload));
  }
}
...
async function deleteSessionController(req, res) {
  ...
  try {
    ...
    if (!deleted) {
      ...
      const errorPayload = createErrorResponse('SESSION_NOT_FOUND', '세션을 찾을 수 없거나 삭제할 수 없습니다. 세션 ID 또는 사용자 ID를 확인해주세요.');
      return res.status(getHttpStatusByErrorCode('SESSION_NOT_FOUND')).json(standardizeApiResponse(errorPayload));
    }
    ...
    const errorPayload = createErrorResponse('SERVER_ERROR', `세션 삭제 중 오류 발생: ${err.message}`);
    res.status(getHttpStatusByErrorCode('SERVER_ERROR')).json(standardizeApiResponse(errorPayload));
  }
}
