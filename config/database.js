...
async function initOracleClient() {
    ...
    if (oracleClientLibDir) {
      try {
        ...
        console.log(`Oracle Thick 모드가 활성화되었습니다. Client Dir: ${oracleClientLibDir}`);
      } catch (err) {
        console.error(`Oracle Client 초기화 실패 (지정된 경로: ${oracleClientLibDir}):`, err);
      ...
      try {
        ...
        console.log('Oracle Thick 모드가 활성화되었습니다. (시스템 기본 경로 또는 설정 없음)');
      } catch (err) {
         console.warn('Oracle Client 초기화 시도 실패 (libDir 없이). Thick 모드가 필요하다면 ORACLE_CLIENT_LIB_DIR 환경 변수를 설정하세요:', err.message);
