...
const { getGeminiTools, executeAiTool, enhancePromptWithTools } = require('../utils/aiTools');
 ...
 */
async function getGeminiApiResponse(currentUserMessage, history, systemMessageText, specialModeType, streamResponseCallback, options = {}, context = {}) {
  if (!genAI) {
  ...
  try {
      ...
      try {
          ...
          if (chunk.candidates && chunk.candidates.length > 0) {
            ...
            if (candidate.content && candidate.content.parts) {
              ...
              if (functionCallParts.length > 0) {
                ...
                console.log(`[GeminiAPI] Function calls detected in stream:`, functionCallParts.length);
        ...
        if (hasFunctionCalls || (finalResult.candidates && finalResult.candidates.length > 0)) {
          ...
          if (candidate.content && candidate.content.parts) {
            ...
            if (functionCallParts.length > 0) {
              console.log(`[GeminiAPI] Processing function calls in streaming mode...`);
              ...
              for (const part of functionCallParts) {
                ...
                console.log(`[GeminiAPI] Executing function: ${name} with args:`, args);
                ...
                try {
                  ...
                  console.error(`[GeminiAPI] Function call error for ${name}:`, toolError);
                  functionResponses.push({
                    functionResponse: {
                      name: name,
                      response: {
              ...
              console.log(`[GeminiAPI] Sending function results back to model (streaming)...`);
              ...
              console.log(`[GeminiAPI] Function calling streaming completed - 총 길이: ${followUpText.length}`);
    ...
    if (candidates && candidates.length > 0) {
      ...
      if (content && content.parts) {
        ...
        if (functionCallParts.length > 0) {
          console.log(`[GeminiAPI] Function calls detected:`, functionCallParts.length);
          ...
          console.log(`[GeminiAPI] Function calling completed - 모델: ${modelName}, 입력 토큰: ${inputTokens}, 출력 토큰: ${outputTokens}`);
