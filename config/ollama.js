 ...
 */
async function getOllamaResponse(modelName, currentUserMessage, history = [], systemMessageText = null, streamResponseCallback = null) {
  const messages = [];
  ...
  try {
    console.log(`Requesting Ollama API with model: ${modelName}, streaming: ${!!streamResponseCallback}`);
    ...
    if (streamResponseCallback) {
      ...
      return new Promise((resolve, reject) => {
        response.data.on('data', (chunk) => {
          try {
            ...
            console.error('Error parsing Ollama stream chunk:', e, chunk.toString());
      ...
      if (response.data && response.data.message && response.data.message.content) {
        ...
        console.error('Invalid non-streaming response structure from Ollama:', response.data);
        return { content: 'Ollama로부터 유효한 응답을 받지 못했습니다.' };
      }
    }
  } catch (error) {
    console.error('Error calling Ollama API:', error.response ? (error.response.data ? error.response.data : error.message) : error.message);
